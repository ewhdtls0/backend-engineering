...
public interface SeatHoldRepository extends JpaRepository<SeatHold, Long> {
    ...
    @Query("select sh from SeatHold sh where sh.seat.id = :seatId and sh.expiresAt > :now and sh.status = SeatHoldStatus.HELD")
    List<SeatHold> findAllAvailableBySeatId(@Param("seatId") Long seatId, @Param("now") LocalDateTime now);

    @Lock(value = LockModeType.PESSIMISTIC_WRITE)
    @Query("select sh from SeatHold sh join fetch sh.seat where sh.id = :seatHoldId")
